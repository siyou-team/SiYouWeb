...
class ApiCtl extends Zero_AppController
{
	...
	public function getAppServerList()
	{
		...
		$app_id = isset($_REQUEST['app_id']) ? intval($_REQUEST['app_id']) : null;

		if (!$app_id)
		{
			$this->dataModel->setError('请输入正确的app id');
			return ;
		}

		$Base_App = new Base_AppModel();
		$base_app_rows = $Base_App->get($app_id);

		if(!$base_app_rows)
		{
			$this->dataModel->setError('App id 不存在');
			return ;
		}

		$Base_AppServerModel = new Base_AppServerModel();
		$app_server_rows = $Base_AppServerModel->getAppServerList(null, $sort, $page, $rows);
		...
		$this->render('user', $app_server_rows, $msg, $status);
	}
	...
	public function bindUserAppServer()
	{
		...
		if (!$user_name)
		{
			$this->dataModel->setError('user_name 不存在');
			return false;
		}

		if (!$server_id)
		{
			$this->dataModel->setError('server_id 不存在');
			return false;
		}
		...
		if(!$base_app_rows)
		{
			$this->dataModel->setError(' $app_id 参数错误');
			return false;
		}
		...
		{
			$this->dataModel->setError(' $server_id 参数错误');
			return false;
		}
		...
		if (is_ok($rs_row) && $Base_App->sql->commit())
		{
			...
			$this->dataModel->addBody(100, $arr_body);
		}
		else
		{
			$Base_App->sql->rollBack();
			$this->dataModel->setError('绑定失败');
		}
	}
	...
	public function addUserAndBindAppServer()
	{
		...
		else
		{
			...
			if ($user_rows)
			{
				$this->dataModel->setError('用户已经存在,请更换用户名!');
				return false;
			}
			else
			{
				...
				$flag = $User_InfoModel->addInfoDetail($arr_field_user_info_detail);
				array_push($rs_row, $flag);
			}
			...
			{
				$Base_App->sql->rollBack();
				$this->dataModel->setError('创建用户信息失败');
			}
		}
	}
	...
	public function addUserAppServer()
	{
		...
		if (!$plantform_url)
		{
			$this->dataModel->setError('plantform_url 不存在--shop-suite');
			return false;
		}
		...
		else
		{
			...
			*/
			...
			//用户是否存在
			$name_hash =  Zero_Hash::hashNum($user_name, 2);
	...
	public function virifyUserAppServer()
	{
		...
		if(!$base_app_server_rows)
		{
			$this->dataModel->setError('App server id 不存在');
			return;
		}
		else
		{
			...
			if (1==$server_state && 0==$base_app_server_row['server_state'])
			{
				...
				{
					$this->dataModel->setError('初始化服务器数据失败!' . json_encode($init_rs));
					return;
			...
			{
				$this->dataModel->setError('请求失败,信息不对');
				return;
			}
			...
			if ($init_flag)
			{
				...
				else
				{
					...
					$this->dataModel->setError('更改状态失败');
				}
			}
			else
			{
				$this->dataModel->setError('操作失败');
			}
		}
	}
	...
	public function getUserAppServerInfo()
	{
		...
		else
		{
			...
			$this->dataModel->addBody(100, $base_app_server_row);
		}
	}
	...
	public function resetPasswd()
	{
		...
		$data         = array();
		$data['user'] = s('user_account');

		if (s('user_password'))
		{
			$data['password'] = md5(s('user_password'));
	...
	public function register()
	{
		...
		$user_name = s('user_account', null);
		$password  = s('user_password', null);
		...
		if (!$user_name)
		{
			$this->dataModel->setError('user_account 不存在');
			return false;
		}

		if (null === $password)
		{
			$this->dataModel->setError('user_password 不能为空');
			return false;
		}
		...
		if ($user_code == $user_code_pre)
		{
			...
			if($app_id && !($base_app_rows = $Base_App->get($app_id)))
			{
				/*
				$base_app_row = array_pop($base_app_rows);
			...
			if (is_ok($rs_row) && $User_InfoModel->sql->commit())
			{
				$d = array();
				$d['user_id'] = $user_id;
				$encrypt_str = Zero_Perm::encryptUserInfo($d, $session_id);
		...
		else
		{
			$msg = '验证码错误';
			$status = 250;
			$this->dataModel->addBody(-1, array(), $msg, $status);
		}
	}
	...
	public function login()
	{
		...
		if(!strlen($user_name))
		{
			$this->dataModel->setError('请输入账号');
		}

		if(!strlen($password))
		{
			$this->dataModel->setError('请输入密码');
		}

		$User_BaseModel = new User_BaseModel();
		$User_InfoModel = new User_InfoModel();
		$user_base_row = $User_BaseModel->getInfoByName($user_name);

		if(!$user_base_row)
		{
			$this->dataModel->setError('账号不存在');
		}
		else
		{

			if(md5($password) != $user_base_row['password'])
			{
				$this->dataModel->setError('密码错误');
			}
			else
			{
				//$session_id = uniqid();
				$session_id = $user_base_row['session_id'];

				$arr_field               = array();
				$arr_field['session_id'] = $session_id;

				//if ($User_BaseModel->editInfo($user_base_row['user_id'], $arr_field) > 0)
				if (true)
				{
					//$arr_body = array("result"=>1,"user_name"=>$user_base_row['user_name'],"session_id"=>$session_id);
					$arr_body = $user_base_row;
					$arr_body['result'] = 1;
					//$arr_body['session_id'] = $session_id;

					$data = array();
					$data['user_id']    = $user_base_row['user_id'];
					//$data['session_id'] = $session_id;
					$encrypt_str        = Zero_Perm::encryptUserInfo($data, $session_id);
	...
	public function getAppServerOld()
	{
		...
		if(!is_int_true($app_id) || !is_int_true($company_id))
		{
			$this->dataModel->setError('参数错误');
		}
		...
		$this->dataModel->setBody($arr_app_server_list);
	}
	...
	public function setAppServerStopTime()
	{
		...
		if(!$arr_app_server)
		{
			$this->dataModel->setError('服务器不存在');
		}

		if(strtotime($server_stop_start_time) > strtotime($server_stop_end_time))
		{
			$this->dataModel->setError('停服开始时间不能大于停服结束时间');
		}
		...
		if ($Base_AppServer->sql->commit())
		{
			...
			$this->dataModel->setBody($arr_body);
		}
		else
		{
			$Base_AppServer->sql->rollBack();
			$this->dataModel->setError(__('操作失败'));
		}
	}
...
//        }
		...
		$this->dataModel->addBody(100, $user_app_server_rows);
	}
	...
	public function activeUserAppServer()
	{
		...
		if(!is_int_true($app_id) || !is_int_true($server_id) || !is_int_true($company_id) || !is_int_true($active_time))
		{
			$this->dataModel->setError('参数错误');
		}
		...
		if(!$arr_app_server)
		{
			$this->dataModel->setError('passport开服列表未配置');
		}
		...
		if(!$find_flag)
		{
			$this->dataModel->setError('找不到服务器id');
		}
		...
		else
		{
			...
			$arr_field['active_time'] = is_int_true($active_time) ? $active_time : time();
			...
			{
				$this->dataModel->setError('数据写入失败');
			}
		}
	}
	...
	public function  checkRecharge()
	{
		...
		if(strlen($order_id))
		{
			$this->dataModel->setError('请传入订单id');
		}
		...
		if(!$arr_pay_verify)
		{
			$this->dataModel->setError('订单id无效');
		}

		if($arr_pay_verify['verify_flag'])
		{
			$this->dataModel->setError('订单已经验证');
		}
		...
		{
			$this->dataModel->setError('充值验证失败');
		}
	}
	*/
	...
	public function version()
	{
		...
		if(!is_int_true($app_id))
		{
			$this->dataModel->setError(__('参数错误'));
		}
		...
		{
			$this->dataModel->setError(__('游戏版本配置表错误'));
		}
		...
		if($app_version != $client_version)//客户端的版本不是最新版本
		{
			...
			{
				$app_pre_version = $base_app_rows_resources['app_pre_version'];
				$base_app_rows_resources_next = $Base_AppResources->getAppResources($app_id, $app_pre_version);
				...
				{
					$version_row['client_version'] = $client_version;
					$version_row['current_version'] = $app_pre_version;
					$version_row['latest_version'] = $base_app_rows_version['app_version'];
		...
		$this->dataModel->setBody($version_row);
	}
