...
class Connect_QqCtl extends Zero_AppController implements Connect_Interface
{
	...
	public function __construct(&$ctl, $met, $typ)
	{
		...
		$this->options = array(
			'appid' => Base_ConfigModel::getConfig('qq_app_id'), //填写高级调用功能的app id
			'appsecret' => Base_ConfigModel::getConfig('qq_app_key'), //填写高级调用功能的密钥
			'cache_name' => 'wechat',//填写缓存目录，默认为当前运行目录的子目录cache下
		);

		$this->api      = new Zero_Api_Wechat($this->options);
	}
	...
	public function login()
	{
		...
		if (Zero_Perm::checkUserPerm())
		{
			...
			$column_row = array(
				'bind_type'=>User_BindConnectModel::WEIXIN,
				'user_id'=>$user_id,
				'bind_active'=> 1
			);
		...
		//子站跳转
		$redirect_url = sprintf('%s?ctl=Connect_Qq&met=callback%s', Zero_Registry::get('url'), LoginModel::callbackStr(true));
		$this->redirectUrl = $redirect_url;

		//
		if (Zero_Utils_Device::isWeixin())
		{
	...
	public function callback()
	{
		...
		if($code)
		{
			...
			if ($access_token_row)
			{
				...
				$bind_id = sprintf('%s_%s', 'qq', $user_info_row['openid']);
                ...
                $User_BindConnectModel = new User_BindConnectModel();
                $flag = $User_BindConnectModel->checkBind($bind_id,  User_BindConnectModel::WEIXIN, $user_id, $data);
			}
			else
			{
				//失败
