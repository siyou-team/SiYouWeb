	...
	return this.each( function() {
			...
			populateVisible: function() {
					...
					(p.lastpage===undefined||(parseInt((tbot + scrollTop + div - 1) / div,10) || 0) <= p.lastpage))
				{
					...
					if (tbot >= 0 || npage < 2 || p.scroll === true) {
						page = $(grid.bDiv).find('tr:not(".jqgfirstrow")').length / p.rowNum + 1;//在行高不固定的情况，原来的算法会出现错误
						ttop = -1;
					} else {
						ttop = 1;
					}
		...
		if(this.p.colNames.length === 0) {
			for (i=0;i<this.p.colModel.length;i++){
				...
				if(this.p.colModel[i].nameExt){
					this.p.colNames[i] += this.p.colModel[i].nameExt;
				}
			}
		}
		...
		refreshIndex = function(sp) {
			...
			if(sp === 'delete') {
				var num = 0;
				for (var b in ts.p._index) {
					if(ts.p._index.hasOwnProperty(b)) {
						ts.p._index[b] = num++;
					}
				};
			} else {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				i++;
				// if(ir===rn) { break; }
				if(ir===rn &&　ts.p.datatype === "local") { break; }//远程给多少就显示多少
			}
			if(ts.p.gridview === true ) {
				fpos = ts.p.treeANode > -1 ? ts.p.treeANode: 0;
				if(ts.p.grouping) {
					...
					$(this.p.colModel).each(function(i){
						...
						if( dval !== undefined) {
							...
							if(t.p.treeGrid===true && nm === t.p.ExpandColumn) {
								...
								var $_td = $("td[role='gridcell']:eq("+i+")",ind);
								var $_input = $_td.find('input');
								if($_input.length){
									vl = vl === '&#160;' ? '' : vl;
									$_input.val(vl).select();
									//由于我们的控件需要和combo控件结合，这里需要处理下不然会出现在编辑combo所在的cell时候保存无效的问题
									if(typeof $_input.getCombo ==='function'){//识别为combo控件生成的元素
										var _combo = $_input.getCombo();
										_combo && _combo.selectByText(vl);
									}
								}else{
									$_td.html(vl);
								}
