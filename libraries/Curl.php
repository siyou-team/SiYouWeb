...
class Curl
{
	...
	protected function log($log)
	{
		if ($this->debug)
		{
			...
			elseif (class_exists('Zero_Log'))
			{
				Zero_Log::log($log, 'curl');
	...
	public function setPostData($url, $postdata, $is_returntransfer = true, $ip = null, $conntimeout = 30, $timeout = 30)
	{
		//SSL安全
	...
	// 从目标地址采集数据
	public function setGetData($url, $is_returntransfer = true, $ip = null, $conntimeout = 30, $timeout = 30)
	{
		//SSL安全
		if (stripos($url, "https://") !== false)
		{
	 ...
	 * @access public
	 */
	function setFetchIntoFile($url, $fp, $ip = null, $timeout = 5)
	{
		//SSL安全
		if (stripos($url, "https://") !== false)
		{
	...
	// 最终结果
	public function exec($try_times = 2)
	{
		for($i=0 ;$i < $try_times; $i++)
		{
			...
			if ($err = curl_errno($this->ch))
			{
				if ($this->debug)
				{
					$this->log("Curl Error try index : $i \n");
					$this->log("Error Occured in Curl\n");
					$this->log("Error number: " . curl_errno($this->ch) . "\n");
					$this->log("Error message: " . curl_error($this->ch) . "\n");
				}
