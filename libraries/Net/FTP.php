...
class Net_FTP extends PEAR
{
    ...
    function Net_FTP($host = null, $port = null, $timeout = 90)
    {
        ...
        $this->_ls_match = array('unix' => array('pattern' => '/(?:(d)|.)([rwxts-]{9})\s+(\w+)\s+([\w\d-()?.]+)\s+' . '([\w\d-()?.]+)\s+(\w+)\s+(\S+\s+\S+\s+\S+)\s+(.+)/', 'map' => array('is_dir' => 1, 'rights' => 2, 'files_inside' => 3, 'user' => 4, 'group' => 5, 'size' => 6, 'date' => 7, 'name' => 8,)), 'windows' => array('pattern' => '/([0-9\-]+\s+[0-9:APM]+)\s+((<DIR>)|\d+)\s+(.+)/', 'map' => array('date' => 1, 'size' => 2, 'is_dir' => 3, 'name' => 4,)));
    }
    ...
    function chmodRecursive($target, $permissions)
    {
        ...
        { // Making directory specific permissions
    ...
    function getRecursive($remote_path, $local_path, $overwrite = false, $mode = null)
    {
        ...
        foreach ($file_list as $file_entry)
        {
            ...
            if ($this->isError($result) && $result->getCode() != NET_FTP_ERR_OVERWRITELOCALFILE_FORBIDDEN)
            {
                return $result;
            }
        }
    ...
    function getExtensionsFile($filename)
    {
        ...
        if (!is_array($exts))
        {
            return $this->raiseError("Extensions-file '$filename' could not be" . "loaded", NET_FTP_ERR_EXTFILELOAD_FAILED);
        }
    ...
    function setDirMatcher($pattern, $matchmap)
    {
        ...
        {
            return $this->raiseError('The supplied pattern is not a string', NET_FTP_ERR_ILLEGALPATTERN);
        }
        if (!is_array($matchmap))
        {
            return $this->raiseError('The supplied pattern is not an array', NET_FTP_ERR_ILLEGALMAP);
        }
        else
        {
            foreach ($matchmap AS $val)
            {
                if (!is_numeric($val))
                {
                    return $this->raiseError('The supplied pattern contains' . 'invalid value ' . $val, NET_FTP_ERR_ILLEGALMAPVALUE);
                }
            }
        }
