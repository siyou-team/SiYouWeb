...
if (!function_exists('ftp_login'))
{
    function ftp_login(&$control, $username, $password)
    {
        ...
        trigger_error('ftp_login() [<a href="function.ftp-login">function.ftp-login' . '</a>]: ' . $contents[count($contents) - 1], E_USER_WARNING);
...
if (!function_exists('ftp_put'))
{
    ...
    {
        if (!is_resource($control) || !is_readable($local) || !is_integer($mode) || !is_integer($pos))
        {
            return false;
        }
...
if (!function_exists('ftp_get'))
{
    ...
    {
        if (!is_resource($control) || !is_writable(dirname($local)) || !is_integer($mode) || !is_integer($resume))
        {
            return false;
        }
        $types   = array(0 => 'A', 1 => 'I');
...
if (!function_exists('ftp_chmod'))
{
    function ftp_chmod(&$control, $mode, $file)
    {
        ...
        trigger_error('ftp_chmod() [<a
            href="function.ftp-chmod">function.ftp-chmod</a>]: ' . rtrim($line), E_USER_WARNING);
        return false;
    }
}
