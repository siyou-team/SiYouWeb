...
(function($) {
  ...
  $.localize = function(options) {
    ...
    deferred = $.Deferred();
    loadLanguage = function(lang, level) {
      var file;
      if (level == null) {
        level = 1;
      }
      ...
      switch (level) {
        case 1:
          ...
          if (options.loadBase) {
            file = ("." + fileExtension);
            return jsonCall(file, lang, level);
          } else {
            return loadLanguage(lang, 2);
          }
          break;
        case 2:
          file = (lang.split('-')[0]) + "." + fileExtension;
          return jsonCall(file, lang, level);
        case 3:
          file = (lang.split('-').slice(0, 2).join('-')) + "." + fileExtension;
          return jsonCall(file, lang, level);
        default:
          return deferred.resolve();
      }
    };
    jsonCall = function(file,lang, level) {
