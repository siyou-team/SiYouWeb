...
var is_join = (!isNull(gb_id) && gb_id != 0) ? true : false;
var map_list = [];
var map_index_id = '';
var store_id;
var activity_id;
        ...
        $.request({
            ...
            success: function (result)
            {
                if (result.status == 200)
                {
                    ...
                    window.activity_id = data.item_row.activity_id
                    ...
                    //倒计时
                    var _timeCount = $(".count-time,.timeCount");
                    ...
                    if (data.item_row.activity_type_id == StateCode.ACTIVITY_TYPE_GROUPBOOKING) {
                    ...
                    //拼单列表
                    {
                        ...
                        $('.swiper-container').css('height',$('.group-book-list').find('.swiper-slide').eq(0).height());
                        ...
                        $("#buy-now").click(function ()
                        {
                            ...
                            $.request({
                                ...
                                success: function (result)
                                {
                                    ...
                                    {
                                        if (window.gb_id == '' && !window.is_join) {
                                            location.href = WapSiteUrl + '/tmpl/order/vr_buy_step1.html?item_id=' + window.item_id + '&quantity=' + buynum;
                                        } else {
                                            location.href = WapSiteUrl + '/tmpl/order/vr_buy_step1.html?item_id=' + window.item_id + '&gb_id=' + window.gb_id + '&activity_id=' + window.activity_id + '&quantity=' + buynum;
                                        }
                                    }
                                }
                            });
                    ...
                    {
                        $("#buy-now,#group-now,#alone-now").click(function ()
                        {
                            var key = getLocalStorage('ukey');//登录标记
                            ...
                            else
                            {
                                ...
                                var json = {};
                                json.key = key;
                                json.gb_id = window.gb_id;
                                json.cart_id = window.item_id + '|' + buynum;
                                json.buytype = buytype;
                                $.request({
                                    type: 'post',
                                    url: SYS.URL.cart.checkout,
                                    data: json,
                                    ...
                                    success: function (result)
                                    {
                                        ...
                                        {
                                            if (window.gb_id == '' && !window.is_join) {
                                                location.href = WapSiteUrl + '/tmpl/order/vr_buy_step1.html?item_id=' + window.item_id + '&quantity=' + buynum + '&buytype=' + buytype;
                                            } else {
                                                location.href = WapSiteUrl + '/tmpl/order/vr_buy_step1.html?item_id=' + window.item_id + '&gb_id=' + window.gb_id + '&activity_id=' + window.activity_id + '&quantity=' + buynum + '&buytype=' + buytype;
                                            }

                                            if (chain_id > 0) {
                                                if (window.gb_id == '' && !window.is_join) {
                                                    location.href = WapSiteUrl + '/tmpl/order/buy_step1.html?if_chain=1&chain_id=' + chain_id +'&item_id=' + window.item_id + '&buynum=' + buynum + '&buytype=' + buytype;
                                                } else {
                                                    location.href = WapSiteUrl + '/tmpl/order/buy_step1.html?if_chain=1&gb_id=' + gb_id + '&activity_id=' + window.activity_id + '&chain_id=' + chain_id +'&item_id=' + window.item_id + '&buynum=' + buynum + '&buytype=' + buytype;
                                                }
                                            } else {
                                                if (window.gb_id == '' && !window.is_join) {
                                                    location.href = WapSiteUrl + '/tmpl/order/buy_step1.html?item_id=' + window.item_id + '&buynum=' + buynum + '&buytype=' + buytype;
                                                } else {
                                                    location.href = WapSiteUrl + '/tmpl/order/buy_step1.html?gb_id=' + gb_id + '&activity_id=' + window.activity_id + '&item_id=' + window.item_id + '&buynum=' + buynum + '&buytype=' + buytype;
                                                }
                                            }
                                        }
                                    }
                ...
                $('.kefu').click(function ()
                {
                    if (data.store_info.im_chat)
                    {
                        window.location.href = WapSiteUrl + '/tmpl/member/chat_info.html?item_id=' + window.item_id + '&user_other_id=' + result.data.store_info.user_id;
                    }
                    else
                    {
...
function virtual()
{
    ...
    $.getJSON(SYS.URL.store.listChainByItem, {store_id: store_id}, function (result)
    {
        if (result.status == 200)
        {
    ...
    $.getJSON(SYS.URL.store.listChainByItem, {store_id: store_id, item_id: item_id}, function (result)
    {
        if (200 == result.status)
        {
            if (result.data.items.length > 0)
            {
                ...
                _html += '<dd>'+__('门店价格') + ': ￥' + chain_map_list[0].chain_item_unit_price +'</dd>';
