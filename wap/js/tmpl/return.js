...
$(function(){
    ...
    $.getJSON(SYS.URL.user.return_item,{order_id:getQueryString('order_id'),order_item_id:getQueryString('order_item_id')}, function(result) {
        ...
        order_item_id = result.data.order.item.order_item_id;
	    ...
	    // 可退金额
        order_item_payment_amount = result.data.order.item.order_item_can_refund_amount;
	    $('input[name="refund_amount"]').val(order_item_payment_amount);
	    $('#returnAble').html('￥'+order_item_payment_amount);
	    
	    // 可退数量
        order_item_quantity = result.data.order.item.order_item_can_return_num;
        ...
        $('.btn-l').click(function(){
            ...
            if (isNaN(param.return_item_num) || parseInt(param.return_item_num) <= 0 || parseInt(param.return_item_num) > parseInt(order_item_quantity)) {
                $.sDialog({
                    skin:"red",
                    content:__('退货数据不能为空，或不能超过可退数量'),
                    okBtn:false,
                    cancelBtn:false
                });
                return false;
