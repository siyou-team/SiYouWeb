var page = pagesize; 
var curpage = 1;
var hasMore = true;
var footer = false;
var reset = true;
var orderKey = "";
var showtype=getQueryString("showtype");
chain_id=getLocalStorage('chain_id');
$(function() {
    if (!ifLogin()){return}

    if (getQueryString("data-state") != "") {
        $("#filtrate_ul").find("li").has('a[data-state="' + getQueryString("data-state") + '"]').addClass("selected").siblings().removeClass("selected")
    }
    $("#search_btn").click(function() {
        reset = true;
        t()
    });
    $("#fixed_nav").waypoint(function() {
        $("#fixed_nav").toggleClass("fixed");
        if (window.suteshopApp)
            {
                $('#fixed_nav.fixed').css({'top': 0});
            }
            else
            {
            }
    },
    {
        offset: "50"
    });
	 
    function t() {
        if (reset) {
            curpage = 1;
            hasMore = true
        }

        if (!hasMore) {
            return false
        }
        hasMore = false;
        var product_state_id = $("#filtrate_ul").find(".selected").find("a").attr("data-state");
        var product_name = $("#goods_key").val();
        $.request({
            type: "post",
            url: SYS.CONFIG.URL.seller.lists_product,
            data: {
                product_state_id: product_state_id,
                product_name: product_name,
                sidx: 'product_add_time',
                sord: 'desc',
                rows:'10',
                page: curpage,
                chain_id:chain_id
            },
            dataType: "json",
            success: function(e) {
                curpage++;
                hasMore = (e.data.page < e.data.total) ? true : false;
                if (!hasMore) {
                    get_footer()
                }
                if (e.data.items.length <= 0) {
                    $("#footer").addClass("posa")
                } else {
                    $("#footer").removeClass("posa")
                }
                var t = e;
                t.WapSiteUrl = WapSiteUrl;
                t.ApiUrl = ApiUrl;
                t.key = getLocalStorage('ukey');
                t.hasmore = hasMore;
                template.helper("$getLocalTime",
                    function (e) {
                        var t = new Date(parseInt(e) * 1e3);
                        var r = "";
                        r += t.getFullYear() + __("年");
                        r += t.getMonth() + 1 + __("月");
                        r += t.getDate() + __("日");
                        r += t.getHours() + ":";
                        r += t.getMinutes();
                        return r
                    });
                template.helper("p2f",
                    function (e) {
                        return (parseFloat(e) || 0).toFixed(2)
                    });
                template.helper("parseInt",
                    function (e) {
                        return parseInt(e)
                    });

                $(".loading").remove();
                var r = template.render("order-list-tmpl", t);
                if (reset) {
                    reset = false;
                    $("#order-list").html(r)
                } else {
                    $("#order-list").append(r)
                }
            }
        })
    }
    $("#order-list").on("click", ".edit-goods", r);
    $("#order-list").on("click", ".offline-goods", o);
    $("#order-list").on("click", ".online-goods", n);
    $("#order-list").on("click", ".delete-goods", l);
    $("#order-list").on("click", ".evaluation-again-order", d);
    $("#order-list").on("click", ".viewdelivery-order", c);
    $("#order-list").on("click", ".check-payment",
    function() {
        var e = $(this).attr("data-paySn");
        toPay(e, "member_buy", "pay");
        return false
    });
    function r() {
        var e = $(this).attr("item_id");
        window.location.href="store_goods_edit.html?item_id="+e;
    }
    function a(r) {
        $.request({
            type: "post",
            url: ApiUrl + "/index.php?act=member_order&op=order_cancel",
            data: {
                order_id: r,

            },
            dataType: "json",
            success: function(e) {
                if (e && e.status == 200) {
                    reset = true;
                    t()
                } else {
                    $.sDialog({
                        skin: "red",
                        content: e.msg,
                        okBtn: false,
                        cancelBtn: false
                    })
                }
            }
        })
    }
    function o() {
        var e = $(this).attr("item_id");
        $.sDialog({
            content: __("是否下架商品？") + "<h6>" + __("下架后还可以在仓库中上架！") + "</h6>",
            okFn: function() {
                i(e)
            }
        })
    }
    function i(r) {
        $.request({
            type: "post",
            url: SYS.CONFIG.URL.seller.edit_state,
            data: {
                product_id: r,
                product_state_id: StateCode.PRODUCT_STATE_OFF_THE_SHELF,
            },
            dataType: "json",
            success: function(e) {
                if (e && e.status == 200) {
                    $.sDialog({
						skin: "red",
						content: __("操作成功"),
						okFn: function() {
							window.location.href = WapSiteUrl + "/tmpl/seller/store_goods_list.html";
						}
					})
                } else {
                    $.sDialog({
                        skin: "red",
                        content: e.msg,
                        okBtn: false,
                        cancelBtn: false
                    })
                }
            },error:function(er){alert(JSON.stringify(er));}
        })
    }
    function n() {
        var e = $(this).attr("item_id");
        $.sDialog({
            content: __("是否上架商品？") + "<h6>" + __("上架后可以正常销售！") + "</h6>",
            okFn: function() {
                s(e)
            }
        })
    }
    function s(r) {
        $.request({
            type: "post",
            url: SYS.CONFIG.URL.seller.edit_state,
            data: {
                product_id: r,
                product_state_id: StateCode.PRODUCT_STATE_NORMAL,
            },
            dataType: "json",
            success: function(e) {
                if (e && e.status == 200) {
                    $.sDialog({
						skin: "red",
						content: __("操作成功"),
						okFn: function() {
							window.location.href = WapSiteUrl + "/tmpl/seller/store_goods_list.html";
						}
					})
                } else {
                    $.sDialog({
                        skin: "red",
                        content: e.msg,
                        okBtn: false,
                        cancelBtn: false
                    })
                }
            },error:function(er){alert(JSON.stringify(er));}
        })
    }
    function l() {
        var e = $(this).attr("item_id");
        $.sDialog({
            content: __("是否删除商品？") + "<h6>" + __("删除后系统不保存！") + "</h6>",
            okFn: function() {
                d(e)
            }
        })
    }
    function d(r) {
       $.request({
            type: "post",
            url: SYS.CONFIG.URL.seller.remove_product,
            data: {
                product_id: r,
            },
            dataType: "json",
            success: function(e) {
                if (e && e.status == 200) {
                    $.sDialog({
						skin: "red",
						content: __("操作成功"),
						okFn: function() {
							window.location.href = WapSiteUrl + "/tmpl/seller/store_goods_list.html";
						}
					})
                } else {
                    $.sDialog({
                        skin: "red",
                        content: e.msg,
                        okBtn: false,
                        cancelBtn: false
                    })
                }
            },error:function(er){alert(JSON.stringify(er));}
        })
    }
    function c() {
        var e = $(this).attr("order_id");
        location.href = WapSiteUrl + "/tmpl/member/order_delivery.html?order_id=" + e
    }
    $("#filtrate_ul").find("a").click(function() {
        $("#filtrate_ul").find("li").removeClass("selected");
        $(this).parent().addClass("selected").siblings().removeClass("selected");
        reset = true;
        window.scrollTo(0, 0);
        t()
    });

	//根据传值显示不同标签
    if (showtype == StateCode.PRODUCT_STATE_NORMAL || !showtype)
        t();
    else
        $("#filtrate_ul a[data-state='" + showtype + "']").click();
    //t();
    $(window).scroll(function () {
        if ($(window).scrollTop() + $(window).height() > $(document).height() - 1) {
            t()
        }
    })
});
function get_footer() {
    if (!footer) {
        footer = true;
        $.request({
            type:'get',
            cache:true,
            url: WapSiteUrl + "/js/tmpl/seller/seller_footer.js",
            dataType: "script"
        })
    }
}