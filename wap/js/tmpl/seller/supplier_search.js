...
$(function(){
    ...
    $('#header-nav').click(function(){
    	if (search_type == 'keyword'){
            if ($('#keyword').val() == '') {
                var data = getLocalStorage('deft_key_value') ? getLocalStorage('deft_key_value') : '';
                window.location.href = WapSiteUrl + '/tmpl/seller/supplier_product_list.html?keyword=' + encodeURIComponent(data);
            } else {
            ...
            if ($('#keyword').val() == '') {
                var data = getLocalStorage('deft_key_value') ? getLocalStorage('deft_key_value') : '';
                window.location.href = WapSiteUrl + '/tmpl/seller/supplier_store_list.html?keyword=' + encodeURIComponent(data);
            } else {
                var data = $('#keyword').val();
                window.location.href = WapSiteUrl + '/tmpl/seller/supplier_store_list.html?keyword=' + encodeURIComponent(data);
            }
