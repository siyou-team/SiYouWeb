    ...
    <div class="sstouch-inp-con">
      <ul class="form-box">
        ...
        <li class="form-item">
          <h4><?=__('退货数量')?></h4>
          <div class="input-box">
            <input type="text" pattern="[0-9]*" class="inp" name="return_item_num" placeholder="<?=__('退货数量不能超过可退数量')?>">
            <span class="input-del"></span> <span class="note"><em id="goodsNum"></em>
            <h6><?=__('最多可退数量')?></h6>
            </span> </div>
        </li>
        <li class="form-item">
	...
	<div class="sstouch-order-item mt5">
		...
		<div class="sstouch-order-item-con">
			<div class="goods-block detail">
				<a href="../../tmpl/product_detail.html?item_id=<%=order.item.item_id%>">
				<div class="goods-pic">
					<img src="<%=order.item.order_item_image%>">
				</div>
				<dl class="goods-info">
					<dt class="goods-name"><%=order.item.item_name%></dt>
					<dd class="goods-type"><%=order.item.spec_info%></dd>
				</dl>
				<div class="goods-subtotal">
					<span class="goods-price">￥<em><%=order.item.order_item_unit_price %></em></span>
					<span class="goods-num">x<%=order.item.order_item_quantity%></span>
				</div>
			</a>
			</div>
		</div>
