...
<header id="header">
  <div class="header-wrap">
    ...
    <span class="header-tab"> <a href="javascript:void(0);" class="cur"><?=__('我的优惠券')?></a> <a href="voucher_pwex.html" class=""><?=__('领取优惠券')?></a> </span>
...
<% for (var k in items) { var v = items[k]; %>
	<li class="ticket-item <% if (v.voucher_state_id == getStateCode().VOUCHER_STATE_UNUSED ) { %>normal<% }else{ %>invalid<%}%>">
		...
		<div class="block-center">
			<div class="store-info">
				<div class="store-avatar"><img src="<%=  %>" alt="" /></div>
				<dl>
					<dt class="store-name"><%= v.store_name %></dt>
					<dd><?=__('有效期至')?>：<%= v.voucher_end_date %></dd>
				</dl>
			</div>
			<div class="ticket-info">
				<div class="bg-ico"></div>
				<% if (v.voucher_state_id == getStateCode().VOUCHER_STATE_USED) { %>
				<div class="watermark ysy"></div>
				<% } %>
				<% if (v.voucher_state_id == getStateCode().VOUCHER_STATE_TIMEOUT || v.voucher_state_id==getStateCode().VOUCHER_STATE_DEL) { %>
				<div class="watermark ysx"></div>
				<% } %>
				<dl>

				<dt><?=__('￥')?><%= v.voucher_price %></dt>
				<dd><% if (v.voucher_subtotal) { %><?=__('满')?><%= v.voucher_subtotal %><?=__('使用')?><% } %></dd>
				</dl>
			</div>
		</div>
