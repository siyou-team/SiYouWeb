...
<link rel="stylesheet" type="text/css" href="../../css/sstouch_member.css">
    <style>
        ...
        .sstouch-order-item-con .goods-info .goods-name{
            height: 1.5rem;
            font-size: .7rem;
            line-height: 1.4rem;
            margin-bottom: .1rem;
        ...
        .sstouch-order-item-con .goods-info{
...
<!--        <input type="button" id="search_btn" value="&nbsp;">-->
        <input type="button" id="scan" value="&nbsp;">
    </form>
  </div>
    ...
    <div class="sstouch-order-item">
        <div class="sstouch-order-item-con">
            <div class="goods-block">
                <div class="goods-pic">
                    <img src="<%= goods_list[i].product_image?goods_list[i].product_image:'../../images/app_new/default.jpg';%>" style="border:solid 2px #ccc;padding:1px;"/>
                </div>
                <dl class="goods-info">
                    <dt class="goods-name"><%=goods_list[i].product_name%></dt>
                    <dd class="goods-type">
                        <?=__('价格')?>€<%=goods_list[i].product_unit_price%>
                        <?=__('会员价')?>€<%=goods_list[i].item_vip_price%>
                        <br/>
                        <?=__('库存')?>：<%= goods_list[i].item_quantity%>
                        <!-- 库存：<%=goods_list[i].goods_storage_sum%>-->
                    </dd>
                </dl>
...
<script>
    function getAppQrcode(data){
        if(!data){
            ...
            window.location.href='./product_edit.php?product_id='+data;
            return false;
        }
    }
    $('#scan').click(function(){
