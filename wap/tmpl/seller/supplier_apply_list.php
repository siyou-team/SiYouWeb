...
<%if(items.length>0){%>
	...
	<%for(var i=0;i<items.length;i++){%>
        <li>
          ...
          <% } else { %>
          <a href="javascript:;" class="oper" data-status = '1' data-id = '<%=items[i].store_distributor_id%>' ><?=__('通过')?></a><a href="javascript:;" data-status = '2' data-id="<%=items[i].store_distributor_id%>" class="oper"><?=__('不通过')?></a>
          <% } %>
