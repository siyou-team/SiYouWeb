...
<div class="goods-detail-cnt">
	...
	<div class="goods-detail-price">
		...
		line-height: .7rem;
		background: #e50001;
		border-radius: .15rem;">送<%=item_row.item_consume_jb;%><?=__('金宝')?></span>
		<%}%>


		<% if (!isEmpty(item_row.analytics_row)){ %>
			<span class="sold"><?=__('销量')?>：<%=item_row.analytics_row.product_sale_num;%> 件</span>
		<%}%>
				...
				<div class="d-content fd-clr">
					...
					<div data-type="rangePrice">
						<dl class="d-price-rangecount" data-index="1">
							<dd><%=wholesale_policy_rows[0].item_quantity_str%> <?=__('起批')?></dd>
	...
	<div class="goods-detail-comment evaluation-enable" id="goodsEvaluation1">
		<div class="title">
			<a id="goodsEvaluation1" href="javascript:void(0);"><?=__('商品评价')?><span class="rate"><?=__('好评率')?><em><%=item_row.analytics_row.evaluation_percent%>%</em></span><span class="rate-num">（<%=item_row.analytics_row.product_evaluation_num%><?=__('人')?><?=__('评价')?>）</span><div class="item-more"><i class="zc zc-arrow-r arrow-r"></i></div></a>
		</div>
	...
	<div class="goods-detail-foot">
		...
		<% if( item_row.store_is_distributor ){ %>
			...
			<a href="javascript:void(0);" class="add-cart" style="width: 100%;"><?=__('已上架')?></a>
			<% } else { %>
			<a href="javascript:void(0);" class="add-cart" id="upload-to" style="width: 100%"><?=__('一键上架')?></a>
			<% } %>

			<% } %>
			
		<% } else { %>
			<a href="javascript:void(0);" class="add-cart" id="apply-to" style="width: 100%"><?=__('申请成为客户')?></a>
		<% } %>
