	...
	<%for(var i=0;i<items.length;i++){%>
        <li>
            <dl class="clearfix">
              ...
              <dd style="font-size: 0.6rem;float: right;line-height: 1.3rem"><%=items[i].store_distributor_time %></dd>
            </dl>
            <div class="handle">
           		<span>
          <% if( items[i].distributor_enable ) { %>
          <a href="javascript:;" class="del" data-id = '<%=items[i].store_distributor_id%>' >删除</a>
          <% } else { %>
          <a href="javascript:;" class="oper" data-status = '1' data-id = '<%=items[i].store_distributor_id%>' >通过</a><a href="javascript:;" data-status = '2' data-id="<%=items[i].store_distributor_id%>" class="oper">不通过</a>
          <% } %>
